...
class WPSEO_Video_Schema {
	...
	public function __construct() {
		...
		add_filter( 'wpseo_schema_article', array( $this, 'filter_article' ), 11, 2 );
		add_filter( 'wpseo_schema_webpage', array( $this, 'filter_webpage' ), 11, 2 );
	}
	...
	public function filter_article( $data, $context ) {
		if ( $this->object->is_needed() ) {
			$data['video'] = array( $this->get_video_id( $context->canonical ) );
		}
	...
	private function get_video_id( $canonical ) {
		return array( '@id' => $canonical . self::VIDEO_HASH );
	}
}
